...
const nbaGemesRefetch = (nowPage: number) => {
  ...
  return getNbaGamesApi({ page: nowPage }).then((res) =>
    res.data.data.sort((a, b) => (dayjs(a.date).isAfter(dayjs(b.date)) ? -1 : 1))
  )
}
...
const Home = () => {
  ...
  const [nowPage, setNowPage] = useState(totalPage + 1)
  ...
  useEffect(() => {
    ...
    setGamesDataState((prev) => prev.concat(gamesData))
  }, [gamesData])
